...
export default function DisguisedLoginPage() {
  ...
  const handleLogin = useCallback(async (user: {username: string, uid: string}) => {
    try {
      ...
      // This is a simplified custom auth flow. In a real app, you'd get this token from a secure server.
      // For this example, we are using a hardcoded placeholder.
      // NOTE: This token is for example purposes and will not work.
      // A real token needs to be generated by the Firebase Admin SDK.
      const placeholderToken = "THIS_IS_A_PLACEHOLDER_JWT";
      
      // In a real app, you would have a server function that generates a token for a given UID.
      // Since we can't do that here, we will have to use anonymous sign-in.
      // We will simulate user creation by writing to the `users` collection.
      
      // For the purpose of this example, we are creating a fake token generation server-side
      // then signing in with that token
